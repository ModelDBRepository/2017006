...
: with updated kinetic parameters from Raman and Bean  _modified to include Q10 factor
: From Raman and Bean 2003
...
PROCEDURE rates(v(mV) )
{
 alfac = (Oon/Con)^(1/4)
 btfac = (Ooff/Coff)^(1/4) 
 f01 = 4 * alpha * exp(v/x1)*qtf
 f02 = 3 * alpha * exp(v/x1)*qtf
 f03 = 2 * alpha * exp(v/x1)*qtf
 f04 = 1 * alpha * exp(v/x1)*qtf
 f0O = gamma * exp(v/x3)*qtf
 fip = epsilon * exp(v/x5)
 f11 = 4 * alpha * alfac * exp(v/x1)*qtf
 f12 = 3 * alpha * alfac * exp(v/x1)*qtf
 f13 = 2 * alpha * alfac * exp(v/x1)*qtf
 f14 = 1 * alpha * alfac * exp(v/x1)*qtf
 f1n = gamma * exp(v/x3)*qtf
 fi1 = Con
 fi2 = Con * alfac
 fi3 = Con * alfac^2
 fi4 = Con * alfac^3
 ...
 b01 = 1 * beta * exp(v/x2)*qtb
 b02 = 2 * beta * exp(v/x2)*qtb
 b03 = 3 * beta * exp(v/x2)*qtb
 b04 = 4 * beta * exp(v/x2)*qtb
 b0O = delta * exp(v/x4)*qtb
 bip = zeta * exp(v/x6)
 b11 = 1 * beta * btfac * exp(v/x2)*qtb
 b12 = 2 * beta * btfac * exp(v/x2)*qtb
 b13 = 3 * beta * btfac * exp(v/x2)*qtb
 b14 = 4 * beta * btfac * exp(v/x2)*qtb
 b1n = delta * exp(v/x4)*qtb
 bi1 = Coff
 bi2 = Coff * btfac
 bi3 = Coff * btfac^2
 bi4 = Coff * btfac^3
